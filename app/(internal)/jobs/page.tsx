...
export default async function JobsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {running.length > 0 && (
        <section>
          <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider mb-3">Running</h2>
      ...
      {queued.length > 0 && (
        <section>
          <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider mb-3">Queued</h2>
          <JobTable jobs={queued} />
        </section>
      )}

      <section>
        <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider mb-3">History</h2>
        <JobTable jobs={done} />
      </section>
    </div>
  );
...
function JobTable({ jobs }: { jobs: Awaited<ReturnType<typeof getJobs>> }) {
  if (jobs.length === 0) {
    return (
      <GlassCard>
        <div className="px-5 py-8 text-center text-white/30 text-sm">None</div>
      </GlassCard>
    );
  }
  return (
