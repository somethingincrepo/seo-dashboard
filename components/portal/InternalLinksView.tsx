...
function DetailPanel({
 ...
 const skipConfirmTimer = useRef<NodeJS.Timeout | null>(null);

 const handleSkip = async () => {
 if (!skipConfirmPending) {
 // First click — arm the confirm. Reset after 4s if user doesn't confirm.
 setSkipConfirmPending(true);
 if (skipConfirmTimer.current) clearTimeout(skipConfirmTimer.current);
 skipConfirmTimer.current = setTimeout(() => setSkipConfirmPending(false), 4_000);
 return;
 }
 if (skipConfirmTimer.current) {
 clearTimeout(skipConfirmTimer.current);
 ...
 <span className="text-slate-400 text-[10px] tracking-wider w-8 shrink-0">From</span>
 <span className="font-mono text-slate-700 truncate">{sourcePath}</span>
 </div>
 <div className="flex items-center gap-3">
 <span className="text-slate-400 text-[10px] tracking-wider w-8 shrink-0">Link</span>
 <span className="text-indigo-600 font-medium truncate">&ldquo;{anchorDisplay}&rdquo;</span>
 </div>
 <div className="flex items-center gap-3">
 <span className="text-slate-400 text-[10px] tracking-wider w-8 shrink-0">To</span>
 ...
 >
 {skipConfirmPending ? "Are you sure?" : "Skip"}
 </button>
 </div>
 )
 ) : null}
 ...
 {proposal?.rationale && (
 <div>
 <div className="text-[11px] font-bold tracking-widest text-slate-400 mb-2">Why we recommend this</div>
 ...
 legacyContext && (
 <div>
 <div className="text-[11px] font-bold tracking-widest text-slate-400 mb-2">Where the link goes in the page</div>
 ...
 <div>
 <div className="text-[11px] font-bold tracking-widest text-slate-400 mb-2">Business impact</div>
 <p className="text-[13px] text-slate-600 leading-relaxed">
 {change.fields.business_impact_explanation}
 </p>
 </div>
