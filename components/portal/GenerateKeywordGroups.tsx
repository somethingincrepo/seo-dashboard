...
export function GenerateKeywordGroups({ token }: { token: string }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = await res.json() as { groups?: GroupPreview[]; error?: string };
      ...
      setError("Failed to generate keyword groups");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/portal/keywords/generate?token=${token}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ save: true, groups: preview }),
      });
  ...
  if (!open) {
    return (
      ...
      >
        ...
        Generate keyword groups with AI
      </button>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-indigo-50 flex items-center justify-between bg-indigo-50/50">
        <div>
          <h3 className="text-[13px] font-semibold text-slate-900">Generate keyword groups</h3>
          <p className="text-[11px] text-slate-400 mt-0.5">Describe your goals — AI will build keyword groups with real volume data</p>
        </div>
        ...
        {!preview && (
          <>
            <textarea
              ...
              placeholder="e.g. We're a dental practice wanting to target patients considering implants, Invisalign, and whitening — add groups focused on each of these services…"
              rows={4}
        ...
        {preview && (
          <>
            <p className="text-[12px] text-slate-500">
              Review the generated groups below. Click <strong>Add all groups</strong> to save them to your keyword list.
            </p>
            <div className="flex flex-col gap-3">
              {preview.map((g) => (
                <div key={g.group} className="rounded-xl border border-slate-200 overflow-hidden">
                  <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                    <p className="text-[13px] font-semibold text-slate-900">{g.group}</p>
                  ...
                  <div className="divide-y divide-slate-100">
                    {g.subkeywords.map((sk) => (
                      <div key={sk.keyword} className="flex items-center justify-between px-4 py-2.5">
                        <span className="text-[13px] text-slate-800 font-medium">{sk.keyword}</span>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {done ? "Added!" : saving ? "Saving…" : `Add ${preview.length} group${preview.length !== 1 ? "s" : ""}`}
              </button>
              <button
