...
export function GroupCard({ group, style, index, token, canDelete }: {
 ...
 <button onClick={() => setShowAdd(true)} className={`text-[10px] font-medium tracking-wider ${style.text} opacity-60 hover:opacity-100 transition-opacity`}>
 + Add keyword
 </button>
 )}
 </div>
...
export function CreateGroupCard({ token, style }: { token: string; style: typeof GROUP_STYLES[0] }) {
 ...
 <div className={`text-[10px] font-bold tracking-widest ${style.text} mb-2`}>New Group</div>
 <input
 type="text"
 value={inputValue}
 onChange={(e) => setInputValue(e.target.value)}
...
export function KeywordGroups({ groups, token }: KeywordGroupsProps) {
 ...
 <div className="text-[10px] font-bold tracking-widest text-slate-400">Groups</div>
 <div className="text-lg font-bold text-slate-900 tabular">{groups.length}</div>
 </div>
 <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex flex-col gap-0.5" style={{ boxShadow: "var(--shadow-xs)" }}>
 <div className="text-[10px] font-bold tracking-widest text-slate-400">Keywords</div>
 <div className="text-lg font-bold text-slate-900 tabular">{totalKeywords}</div>
 </div>
 {avgKd !== null && (
 <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex flex-col gap-0.5" style={{ boxShadow: "var(--shadow-xs)" }}>
 <div className="text-[10px] font-bold tracking-widest text-slate-400">Avg Difficulty</div>
